...
class RosAriaNode
{
  ...
  public:
    ...
    void setpose_cb( const nav_msgs::OdometryConstPtr &);
...
int RosAriaNode::Setup()
{
  ...
  setpose_sub = n.subscribe( "setPose", 1, (boost::function <void(const nav_msgs::OdometryConstPtr&)>)
      boost::bind(&RosAriaNode::setpose_cb, this, _1));
...
void RosAriaNode::setpose_cb( const nav_msgs::OdometryConstPtr &msg)
{

veltime = ros::Time::now();
  ROS_INFO( "Global pose: [%0.2f,%0.2f,%0.2f](%0.3f)", msg->pose.pose.position.x, msg->pose.pose.position.y , msg->pose.pose.orientation.w , veltime.toSec() );
  odPos.setX(msg->pose.pose.position.x*1000);
  odPos.setY(msg->pose.pose.position.y*1000);
  odPos.setTh(msg->pose.pose.orientation.w*180/M_PI);
  robot->lock();
  robot->moveTo(odPos);
  robot->unlock();
  ROS_DEBUG("RosAria: sent pose to to aria (time %f): x %f, y %f, ang %f deg/s", veltime.toSec(),
    (double) msg->pose.pose.position.x, (double) msg->pose.pose.position.y, (double) msg->pose.pose.orientation.w*180/M_PI);
